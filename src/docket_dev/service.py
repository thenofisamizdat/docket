...
def read_project_config(root: str | Path) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "dev_mode": agent.get("dev_mode", "pr"),
        "auto_deploy": bool((data.get("deploy") or {}).get("auto", False)),
        "deploy_cmd": (data.get("deploy") or {}).get("cmd", ""),
    }


def update_project_config(root: str | Path, *, base_branch: Optional[str] = None,
                          dev_mode: Optional[str] = None,
                          auto_deploy: Optional[bool] = None,
                          deploy_cmd: Optional[str] = None) -> Dict[str, Any]:
    """Edit specific keys in <root>/.docket/config.toml by line replacement.
    The file is always generated by config.to_toml, so each edited key appears
    exactly once ([repo].base_branch, [agent].dev_mode, [deploy].auto/cmd; a
    pre-deploy-era file gets the [deploy] section appended). Running services
    must be restarted to pick changes up (the agent reads config at startup)."""
    path = Path(root) / ".docket" / "config.toml"
    if not path.is_file():
        raise ValueError("project has no .docket/config.toml")
    text = path.read_text()
    if base_branch is not None:
        text, n = re.subn(r'(?m)^base_branch = ".*"$',
                          f'base_branch = "{_esc(base_branch)}"', text, count=1)
        if not n:
            raise ValueError("config.toml has no base_branch line")
    if dev_mode is not None:
        if dev_mode not in DEV_MODES:
            raise ValueError(f"dev_mode must be one of {DEV_MODES}")
        text, n = re.subn(r'(?m)^dev_mode = ".*"$',
                          f'dev_mode = "{_esc(dev_mode)}"', text, count=1)
        if not n:
            raise ValueError("config.toml has no dev_mode line")
    if auto_deploy is not None or deploy_cmd is not None:
        if "[deploy]" not in text:
            text = text.rstrip() + '\n\n[deploy]\nauto = false\ncmd = ""\n'
        if auto_deploy is not None:
            text = re.sub(r'(?m)^auto = (?:true|false)$',
                          f"auto = {str(bool(auto_deploy)).lower()}", text, count=1)
        if deploy_cmd is not None:
            text = re.sub(r'(?m)^cmd = ".*"$',
                          f'cmd = "{_esc(deploy_cmd)}"', text, count=1)
...
def list_branches(root: str | Path) -> List[str]:
    """Local + remote branch names of the project repo (deduped, remotes
    stripped of the remote prefix), for the merge-branch picker."""
    ...
    for line in (r.stdout or "").splitlines():
        ...
        if not line or line.endswith("/HEAD"):
            continue
        if "/" in line and line.split("/", 1)[0] in ("origin", "upstream"):
            line = line.split("/", 1)[1]
